...
export function ListMaintenances() {
  const { fetchMaintenances, maintenances } = useMaintenance()
  ...
  return (
    <div className="overflow-auto mb-20">
      <table className="w-full">
        <thead>
          <tr>
            <th className="px-16 text-xs rounded-tl-lg text-left">Usuário</th>
          ...
          {maintenances.map((maintenance) => {
            return (
              <tr key={maintenance.id}>
                <td className="px-16  flex flex-col leading-4">
                ...
                <td className="">{formatDate(maintenance.maintenanceDate)}</td>
                <td className="">
                ...
                <td className="flex gap-2 items-center justify-center py-2 mt-1">
                  <Dialog.Root>
                    ...
                    {onOpen && (
                      <UpdateMaintenanceModal maintenance={maintenanceToEdit} />
                    )}
                  </Dialog.Root>
